...
data = csvread('scaledData.csv', 1); %add file path correctly
...
net = feedforwardnet([16],'traingdm'); % parameters: number of neurons in hidden layer, 
                                    %training function: traingdm  (gradient decent with momentum) back propagation neural network

net.trainParam.lr = 0.01; % alpha value (used in sigmoid activation function (feed forward) and calculating the gradients (back propagation))
net.trainParam.mc = 0.5; % momentum value (used in back propagation during weight updation)
net.trainParam.min_grad = 3e-4; %eta value (used in weight updation)
